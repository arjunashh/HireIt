...
export default function InterviewRoom({ session, onReport, onBack }) {
  ...
  async function joinZoom() {
    ...
    try {
      ...
      // Use the globally loaded CDN version instead of NPM module
      const ZoomMtgEmbedded = window.ZoomMtgEmbedded;

      if (!ZoomMtgEmbedded) {
        throw new Error("Zoom CDN not loaded. Check index.html script tags.");
      }

      const client = ZoomMtgEmbedded.createClient();
      client.init({ zoomAppRoot: zoomRef.current, language: "en-US" });
      await client.join({ sdkKey: sdk_key, signature, meetingNumber: meeting.meeting_id, password: meeting.password, userName: "Interviewer" });
      setZoomJoined(true);
    } catch (err) { setZoomError("Zoom join failed: " + err.message); }
